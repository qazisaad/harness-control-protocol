...
2. The control plane returns `request_id`, `pairing_code`, `pairing_url`, `expires_at`, and `poll_interval_seconds`. The CLI prints instructions before polling. A display code cannot exchange credentials.
3. An authenticated organization administrator approves or declines the request in the consuming product. The runner posts `request_id`, its exchange secret, and the same identities/protocol to `/pairing-exchange`.
